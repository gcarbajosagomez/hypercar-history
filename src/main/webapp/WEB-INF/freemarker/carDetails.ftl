  						...
  						<div class="carousel-inner">    						
    						<#list pictureIds as pictureId>
    							<div class="item <#if pictureId_index == 0> active </#if>" id="pic-div-${pictureId}">
      								<a href="${pictureURL}${HTMLSuffix}?${action}=${loadCarPictureAction}&${picId}=${pictureId}" title="${car.manufacturer.name} ${car.model}" data-gallery>
										<img src="${pictureURL}${HTMLSuffix}?${action}=${loadCarPictureAction}&${picId}=${pictureId}" alt="${car.manufacturer.name} ${car.model}"> 
									</a> 														
    							</div>
	      					</#list>
		    			</div>					
