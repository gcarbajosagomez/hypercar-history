	...
	<div class="row">
		<div class="col-lg-2">
			<div class="thumbnail list-group cars-list-group">
				<#list models as car>
    				<a class="list-group-item" href=${carEditURL}${HTMLSuffix}?${carId}=${car.id}>
		...
		<div id="main-car-list-div" class="col-lg-10 thumbnail">
			<#if (itemsPerPageData >= 1)>
				<div class="col-lg-12" id="car-list-div">	
						...
						<div id="pagination-row-div" class="row">
							...
							<div class="<#if (chunkedModelsList?size == 1)>text-center<#else>text-right</#if> <#if (chunkedModelsList?size < 2)>col-lg-12 col-md-12 col-sm-12 col-xs-12<#else><#if (chunkedModelsList?size < 3)>col-lg-5<#else>col-lg-4</#if> col-md-5 col-sm-5 col-xs-12</#if>" style="height:56px; margin-bottom: 20px;">
								<button class="btn btn-default dropdown-toggle" style="padding: 10px; margin-top: 15px" type="button" id="cars-per-page-menu" data-toggle="dropdown">
    								${getTextSource('itemsPerPage')}
    								<span class="caret"></span>
  								</button>
  								<ul class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="cars-per-page-menu">
									<li role="presentation"><a role="menuitem" href="${carListURL}${HTMLSuffix}?${pagNum}=1&${itemsPerPage}=5">5</a></li>
    								<li role="presentation"><a role="menuitem" href="${carListURL}${HTMLSuffix}?${pagNum}=1&${itemsPerPage}=10">10</a></li>
    								<li role="presentation"><a role="menuitem" href="${carListURL}${HTMLSuffix}?${pagNum}=1&${itemsPerPage}=15">15</a></li>
    								<li role="presentation"><a role="menuitem" href="${carListURL}${HTMLSuffix}?${pagNum}=1&${itemsPerPage}=20">20</a></li>
   									<li role="presentation" class="divider"></li>
	    							<li role="presentation"><a role="menuitem" href="${carListURL}${HTMLSuffix}?${pagNum}=1&${itemsPerPage}=${models?size}">${getTextSource('allCars')}</a></li>
  								</ul>
  							</div>
  						</div>
  					</div>	
...
<script type='text/javascript'>
	         		...
	         		{	         			
    	     	    	...
    	     	    	$.ajax({
                				...
                				beforeSend: function(xhr)
                				{
									...
									xrh = addCrsfTokenToAjaxRequest(xhr);
  								}
	                	  	  })	                	 
	                	  	  .done(function (data)
						  	  {    	            	 
        	        	 	    	...
        	        	 	    	if (data != null)
        	        	 	    	{
        	        	 	    		window.history.pushState(null,'',"${carListURL}${HTMLSuffix}?${pagNum}=" + data.pagNumData + "&${itemsPerPage}=" + data.itemsPerPageData); 
									}
						  	  });  
					}                      
            	}
    ...
    { 	
    	...
    	for (var i=0 ; i< cars.length; i++)
    	{
            ...
            carRowString = carRowString.concat(					"<img class='img-thumbnail car-preview-img' src='/pagani-history-web/${pictureURL}${HTMLSuffix}?${action}=${loadCarPreviewAction}&${carId}=" + cars[i].id + "' alt='" + cars[i].manufacturer.name + cars[i].model + "'>");
            carRowString = carRowString.concat(				"</div>");
            carRowString = carRowString.concat(				"<figcaption>");
			carRowString = carRowString.concat(					"<a href='${carEditURL}${HTMLSuffix}?${carId}=" + cars[i].id + "' style='padding-bottom: 0px; padding-top: 0px;'>");
    	...
    	<li style="z-index: <#if zIndex??>${zIndex}<#else>1</#if>">
        	<figure>
				<div class="caption vertically-aligned-div vertically-aligned-car-preview-div">
               		<img class="img-thumbnail car-preview-img" src="/pagani-history-web/${pictureURL}${HTMLSuffix}?${action}=${loadCarPreviewAction}&${carId}=${car.id}" alt="${car.manufacturer.name} ${car.model}">
				</div>
				<figcaption>
			 		<a href="${carEditURL}${HTMLSuffix}?${carId}=${car.id}" style="padding-bottom: 0px; padding-top: 0px;">
