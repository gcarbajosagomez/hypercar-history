...
<script type='text/javascript'>
        	...
        	onPageClicked:
	         	...
	         	{      		
	         		...
	         		{	         			
    	     	    	...
    	     	    	$.ajax({
        	        			type: 'POST',
            	    			url: '<@spring.url "/${cmsContext}${carsURL}${HTMLSuffix}"/>',
                				dataType: 'json',
    ...
    { 	
    	...
    	for (var i=0 ; i< cars.length; i++)
    	{
            ...
            carRowString = carRowString.concat(					"<img class='img-thumbnail preview-img' src='/pagani-history-web/${pictureURL}${HTMLSuffix}?${action}=${loadCarPreviewAction}&${carId}=" + cars[i].id + "' alt='" + cars[i].manufacturer.name + cars[i].model + "'>");
    	...
    	<li style="z-index: <#if zIndex??>${zIndex}<#else>1</#if>">
        	<figure>
				<div class="caption vertically-aligned-div vertically-aligned-preview-div">
               		<img class="img-thumbnail preview-img" src='<@spring.url "/${pictureURL}${HTMLSuffix}?${action}=${loadCarPreviewAction}&${carId}=${car.id}"/>' alt="${car.manufacturer.name} ${car.model}">
				</div>
				<figcaption>
